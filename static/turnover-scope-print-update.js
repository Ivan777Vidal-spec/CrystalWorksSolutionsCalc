(()=>{
  ...
  style.textContent=`
    @media print{
      ...
      .doc table{font-size:10.5px!important}
      .doc th,.doc td{padding:6px 7px!important;font-size:10.5px!important}
      .cws-banner{gap:6px!important;margin:8px 0 12px!important}
      .cws-pill{padding:5px 8px!important;font-size:10px!important}
      .cws-summary-sections{gap:9px!important;margin-top:9px!important}
      .cws-summary-section{padding:10px!important;border-radius:10px!important}
      .cws-check{font-size:10.5px!important;margin:3px 0!important}
      .cws-doc-note{font-size:10px!important;line-height:1.28!important}
      #customerSummaryView h2{margin-top:13px!important}
      #customerSummaryView .proposal-grid{gap:9px!important}
  ...
  function refreshCustomerScope(){const target=$('csScope');if(!target)return;const sections=scopeSections();target.innerHTML=Object.entries(sections).map(([heading,items])=>`<div class="cws-summary-section"><h3>${heading}</h3>${items.map(item=>`<div class="cws-check">${item}</div>`).join('')}</div>`).join('');}
